...
public class SetRangeSum {
    ...
    void erase(int x) {
        ...
        VertexPair split1 = split(root, x);
        if (split1.right == null || split1.right.key != x) return;
        VertexPair split2 = split(split1.right, x + 1);
        root = merge(split1.left, split2.right);
    }

    boolean find(int x) {
        // Implement find yourself
        VertexPair element = find(root, x);
        Vertex result = element.left;
        root = element.right;
        if (result == null)
            return false;
        return result.key == x;
    ...
    long sum(int from, int to) {
        ...
        long ans = middle == null ? 0 : middle.sum;
